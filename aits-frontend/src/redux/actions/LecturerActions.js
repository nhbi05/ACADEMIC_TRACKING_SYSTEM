...
export const fetchAssignedIssues = () => async (dispatch) => {
  dispatch({ type: 'FETCH_ASSIGNED_ISSUES_REQUEST' });
  
  try {
    const issues = await lecturerService.getAssignedIssues();
    dispatch({ 
      type: 'FETCH_ASSIGNED_ISSUES_SUCCESS', 
      payload: issues 
    });
  } catch (error) {
    dispatch({ 
      type: 'FETCH_ASSIGNED_ISSUES_FAILURE', 
...
export const fetchResolvedIssues = () => async (dispatch) => {
  dispatch({ type: 'FETCH_RESOLVED_ISSUES_REQUEST' });
  
  try {
    const resolvedIssues = await lecturerService.getResolvedIssues();
    dispatch({ 
      type: 'FETCH_RESOLVED_ISSUES_SUCCESS', 
      payload: resolvedIssues 
    });
  } catch (error) {
    dispatch({ 
      type: 'FETCH_RESOLVED_ISSUES_FAILURE', 
      payload: error.message 
    });
  }
};
...
export const resolveIssue = (issueId) => async (dispatch) => {
  dispatch({ type: 'RESOLVE_ISSUE_REQUEST' });
  
  try {
    await lecturerService.resolveIssue(issueId);
    
    // After resolving, fetch both assigned and resolved issues to update the UI
...
export const fetchIssueDetails = (issueId) => async (dispatch) => {
  dispatch({ type: 'FETCH_ISSUE_DETAILS_REQUEST' });
